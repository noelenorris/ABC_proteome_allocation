    ...
    for it=1:1:num_results
        ...
        % Calculate Michaelis-Menten transport parameters
           ...
           %CONCENTRATIONS IN THE CYTOPLASM
            ...
            BP_periplasm{it} = BP_peri;
            T_periplasm{it} = T_peri;
            ...
            %Calculate V_max
              ...
              if(transport_model == 1) %ABC 
               ...
               if(transport_model == 1)
                   ...
                   for it2=1:1:length(BP_peri)
                       ...
                       + S_p2.*T_total*k2*k0f*k1f + S_p2.*T_total*k3*k0f*k1f))./(2*k1f*(k2 + k3)*(k2*k3 + S_p2.*k2*k0f + S_p2.*k3*k0f));
                        ...
                        if(~isnan(uptake_K_eff(1)))
                            ...
                            f_uptake = fitnlm(S_p2', (uptake_K_eff/V_max_T{it}(it2))', 'y ~ x/(b1+x)', 10^(-5));

                            K_eff_it(it2) = f_uptake.Coefficients.Estimate;
                        else
                            K_eff_it(it2) = NaN;
                        end
                   ...
                   K_T = k2*k3/(k1*(k2+k3));
                   K_D = k0r/k0f;
                   K_approx{it} = K_T*K_D./(K_T + BP_periplasm{it});

               else
                   K_eff{it} = k2/k1*ones(size(b1));
                   K_approx{it} = K_eff{it};
               end
...
%% ADDITIONAL PLOTS: ABC CELL, PERIPLASMIC VOLUME FRACTION

            periplasm_factor = 10.^[-4:.01:4];
            
            for i=1:1:length(periplasm_factor)
                
                ratio_vol_cyto_peri = (1-vol_ratio_periplasm{2}(1))./vol_ratio_periplasm{2}(1)/periplasm_factor(i);
                
                %Calculuate V_max 
                    S_p2 = 10^10;

                BP_total = BP_periplasm{2}(1);
                T_total = T_periplasm{2}(1)/periplasm_factor(i);
                T_total_it(i) = T_total;
                k0r = 100/1000; %msec-1
                ...
                T_S_BP = (k3.*(k2.*k3.*k0r - (BP_total.^2.*S_p2.^2.*k2.^2.*k0f.^2.*k1f.^2 + 2.*BP_total.^2.*S_p2.^2.*k2.*k3.*k0f.^2.*k1f.^2 + BP_total.^2.*S_p2.^2.*k3^2*k0f^2*k1f^2 ...
                 ...
                 % cytoplasmic transport rate ([S]_p --> S_c)
                 Vmax(i) = k2*T_S_BP./ratio_vol_cyto_peri;





            %Calculate K_M and affinity of ABC transport around optimal solution at 1 nM.
             S_p2 = 10.^[-10:.01:-2];
                   ...
                   + S_p2.*T_total*k2*k0f*k1f + S_p2.*T_total*k3*k0f*k1f))./(2*k1f*(k2 + k3)*(k2*k3 + S_p2.*k2*k0f + S_p2.*k3*k0f));
                    ...
                    uptake_K_eff = k2*T_S_BP/ratio_vol_cyto_peri;
                    ...
                    if(~isnan(uptake_K_eff(1)))
                        ...
                        %f_uptake = fit(S_p2', (uptake_K_eff/V_max_T{it}(it2))', 'x/(a+x)', 'Lower', [0], 'Upper', [10^(-3)], 'StartPoint', [10^(-5)]);
                        f_uptake = fitnlm(S_p2', (uptake_K_eff/Vmax(i))', 'y ~ x/(b1+x)', 10^(-5));

                        K_M(i) = f_uptake.Coefficients.Estimate;
                        
                        affinity(i) = Vmax(i)/K_M(i);
                        affinity_approx(i) = k1*T_total*BP_total/K_D/ratio_vol_cyto_peri;
                        
                    else
                        K_M(i) = NaN;
                    end
                 
                % Calculate proportion of bound transport units for [S]_p = 1 nM.    
                 S_p2 = 10.^[-6];
        ...
        figure()        
        loglog(S{2}, S_c{2},'-o','LineWidth',8, 'Color', col(4,:))
        hold on
        loglog(S{2}, A{2},'-o','LineWidth',8, 'Color', col(5,:))
        loglog(S{2}, P{2}.*(phi_E{2}+phi_R{2}+phi_M{2}),'-o','LineWidth',8, 'Color', col(6,:))
        xlabel('Nutrient concentration [mM]')
        ylabel('Optimal cytoplasmic concentration [mM]', 'FontSize', 16)
        xticks([10^(-6) 10^(-4) 10^(-2) 10^0 10^2])
        legend('S_c', 'Amino acids', 'Protein')
        grid on
        set(findall(gcf,'-property','FontSize'),'FontSize',18)
        hold off
    
        figure()
        semilogx(S{2}, BP_periplasm{2},'-o','LineWidth',8, 'Color', col(4,:))
        hold on
        semilogx(S{2}, T_periplasm{2},'-o','LineWidth',8, 'Color', col(5,:))
        semilogx(S{2}, P{2}.*phi_T{2}/num_aa_T, '-o','LineWidth',8, 'Color', col(6,:))
        xlabel('Nutrient concentration [mM]')
        ylabel('Optimal concentration [mM]', 'FontSize', 16)
        xticks([10^(-6) 10^(-4) 10^(-2) 10^0 10^2])
        legend('[BP]_{total}', '[T]_{total}', '[T]_{total}V_{peri}/V_{cyto}')
        set(findall(gcf,'-property','FontSize'),'FontSize',18)
        grid on 
        hold off
        ...
        figure()
        % Optimal BP + T concentrations
        subplot(1,2,1)
        semilogx(periplasm_factor,T_S_BP_it./T_total_it,'-', 'LineWidth',8, 'Color', col(4,:))
        xlabel('Factor change in periplasmic volume', 'FontSize', 16)
        xticks([10^(-4) 10^(-2) 10^(0) 10^(2) 10^(4)])
        ylabel('[T:S:BP]/[T]_{total}', 'FontSize', 16)
        set(findall(gcf,'-property','FontSize'),'FontSize',18)
        grid on
        hold off
        ...
        subplot(1,2,2)
        semilogx(periplasm_factor,10^6*K_M,'-', 'LineWidth',8, 'Color', col(4,:))
        hold on
        semilogx(periplasm_factor,10^6*K_approx{2}(1)*ones(size(periplasm_factor)),'-', 'LineWidth',6, 'Color', col(5,:))
        xlabel('Factor change in periplasmic volume', 'FontSize', 16)
        xticks([10^(-4) 10^(-2) 10^(0) 10^(2) 10^(4)])
        ylabel('K_M [nM] ', 'FontSize', 16)
        ylim([0 25])
        legend('Calculated', 'Approximation')
        yticks([0 5 10 15 20 25])
        set(findall(gcf,'-property','FontSize'),'FontSize',18)
        grid on
        hold off
